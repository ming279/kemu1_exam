...
### 核心功能
- **模拟考试**：随机组卷 100 题（判断题 1–40 + 单选题 41–100 分区展示），左侧答题卡实时标记作答状态，交卷自动判分生成成绩单；**作答实时自动保存**（防抖落库 + pagehide beacon 兜底），刷新/误点导航/关浏览器均不丢答案，再次进入自动恢复未完成试卷并回显已答选项
- **练习中心**：三种模式共用同一套答题链路——🎲 随机练习（全库随机抽题）、🎯 专项练习（按知识分类父/子类 + 题型筛选抽题，题量自填 1–500）、❌ 错题练习（只抽未掌握错题）；逐题作答即时反馈，答错自动进入错题本
- **错题本**：自动汇总错题及错误次数；错题连对 2 次才自动标记掌握移出（答错立即清零），支持手动"标记已掌握"
- **考试记录**：独立"考试记录"页列出本人全部试卷（开考时间标识，无全站卷号困惑），可继续作答未完成试卷或回看成绩单/解析，支持按来源大类（模拟考试/任务）筛选；"我的统计"页提供总场次/已完成/进行中/平均分概览
...
| ⑦ | **双人 PK 赛车** | 基于 flask-socketio 的实时对战：发起方可自定义题型（判断题/单选题各若干，合计 10 题、交错出场）、15 秒/题、答对小车前进、3-2-1 红绿灯倒计时、快捷表情包、胜利烟花、抢答答错直接送对方 1 分并跳下一题、胜方段位战绩更新；赛道由 Canvas 手绘引擎渲染（F1 风格赛车与车道**按角色固定绑定**：发起方永远左道红车、应战方右道蓝车，双方画面一致），含氮气尾焰/冒烟/COMBO/超车音效/Web Audio 合成音效；服务监听 `0.0.0.0` 支持公网/局域网双人联机 |
...
│       ├── pk_lobby.html / pk_room.html  # ⑦ 双人 PK 大厅（题型配置）/ Canvas 赛车房间
│       ├── practice_home.html / practice.html # 练习中心（随机/专项/错题）/ 答题页
│       ├── my_tasks.html          # ② 我的任务（待完成/已完成两块）
│       ├── my_papers.html         # 考试记录全列表（来源筛选）
...
| 学生 | 模拟考试、练习（随机/专项/错题）、错题本、我的任务、考试记录、我的统计、学生排名、PK 挑战 |
...
| PK 对战 | 题型由发起方配置：判断题/单选题各若干、合计 10 题（大厅表单实时校验），抽题后交错出场、每题 15 秒；答对 +1 分并锁定该题（对手再答无效），抢答答错直接送对方 1 分并立即进入下一题；答完 10 题比总分，平分判平局（双方胜/负场均不变）；车道/车型按角色固定（发起方左道红车、应战方右道蓝车） |
...
- 14 图分布：错题排行榜 4（TOP10 错误率条形/题型玫瑰图/错误答案环形/错误率区间饼）、全局统计 5（14 天双系列面积折线/正确率仪表盘/题型环形/分类条形/活跃堆叠柱）、我的统计 3（个人仪表盘/成绩走势折线带 90 分参考线/易错题条形）、排名 2（分数段渐变柱，按 90 分及格口径分 4 桶 / PK 胜负环）
...
- 考试组卷比例：判断题 40 + 单选题 60，共 100 题，与真实科目一规则一致（90 分及格）；多选题仅在练习中出现
- 练习中心分随机/专项/错题三种模式，题卡由后端生成；考试作答实时自动保存，误点导航或刷新不丢答案，未完成试卷再次进入自动恢复
- 任务抽题在发布时固定题目列表：考试用途按学生 ID 做随机种子打乱顺序（判断题始终在前）防邻座作弊；讲解用途全班同序便于统一讲评；任务可定向发布给指定学生
- PK 对战题型由发起方配置（判断/单选合计 10 题），抢答答错直接送对方 1 分并跳下一题；车道车型按角色固定，双方画面一致；房间状态存于服务进程内存（生产 gevent / 开发 threading），服务重启后进行中的对战会中断，已完成战绩已落库不受影响
